    ...
    <div class="max-w-2xl">
        ...
        <div class="flex items-center gap-3 mb-6">
            ...
            <div>
                ...
                <flux:text class="text-sm text-zinc-500">{{ __('Define a new role and assign permissions to it.') }}</flux:text>
            </div>
        </div>
            ...
            <form wire:submit="save" class="divide-y divide-zinc-200 dark:divide-zinc-700">
                ...
                @if ($allPermissions->isNotEmpty())
                    <div class="p-6">
                        <div class="flex items-center justify-between mb-3">
                            <flux:text class="text-xs font-semibold uppercase tracking-wider text-zinc-400">{{ __('Permissions') }}</flux:text>
                            <div class="flex items-center gap-2">
                                @if (count($selectedPermissions) === $allPermissions->count())
                                    <flux:button size="xs" variant="subtle" wire:click="deselectAll">
                                        {{ __('Deselect All') }}
                                    </flux:button>
                                @else
                                    <flux:button size="xs" variant="subtle" wire:click="selectAll">
                                        {{ __('Select All') }}
                                    </flux:button>
                                @endif
                                <flux:text class="text-xs text-zinc-400">{{ count($selectedPermissions) }}/{{ $allPermissions->count() }}</flux:text>
                            </div>
                        </div>
                ...
                <div class="flex items-center gap-3 p-6">
                    <flux:button type="submit" variant="primary" wire:loading.attr="disabled" wire:target="save">
                        <span wire:loading.remove wire:target="save">{{ __('Create Role') }}</span>
